...
export function StockAnalystPhaseDetails({ analysisData }: StockAnalystPhaseDetailsProps) {
  return (
        ...
        <CardContent>
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            <div className="p-3 rounded-lg bg-muted/50">
              ...
              <p className="text-2xl font-bold">{analysisData?.stocksAnalyzed?.length ?? "—"}</p>
            </div>
            <div className="p-3 rounded-lg bg-muted/50">
              <p className="text-xs text-muted-foreground">Recommendations</p>
              <p className="text-2xl font-bold">{analysisData?.recommendations?.length ?? 0}</p>
            </div>
          </div>
