...
import { pipelineHistoryQuery, currentPipelineQuery, latestRunQuery } from "../queries"
    ...
    <Card className="border-blue-500/30 bg-blue-500/5">
      ...
      <CardContent className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <span className="font-medium">{PHASE_LABELS[current.phase] ?? current.phase}</span>
        </div>
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex items-center justify-between text-sm">
          <span className="text-muted-foreground">{completedPhases}/{totalPhases} phases</span>
          {durationStr && <span className="text-muted-foreground">Duration: {durationStr}</span>}
        </div>
      </CardContent>
    </Card>
  )
